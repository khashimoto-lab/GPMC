...
// flow-cutter headers do not include their own dependencies; they must be
// listed in dependency order, not alphabetically (e.g. separator.h needs
// union_find.h to come first).
#include "extern/flow-cutter-pace17/src/multi_arc.h"
#include "extern/flow-cutter-pace17/src/union_find.h"
#include "extern/flow-cutter-pace17/src/separator.h"
#include "extern/flow-cutter-pace17/src/id_multi_func.h"
#include "extern/flow-cutter-pace17/src/filter.h"
#include "extern/flow-cutter-pace17/src/contraction_graph.h"
#include "extern/flow-cutter-pace17/src/greedy_order.h"
#include "extern/flow-cutter-pace17/src/heap.h"
...
void check_multilevel_partition_invariants(const Tail& tail, const Head& head,
    ...
    #else
    (void)tail; (void)head; (void)multilevel_partition;
    #endif
}

template<class Tail, class Head, class ComputeSeparator, class ShouldStop, class OnNewMP>
void compute_multilevel_partition(const Tail& tail, const Head& head,
    ...
    int materialized_tw = smallest_known_treewidth;
    ...
    auto materialize_best = [&] {
        if (smallest_known_treewidth < materialized_tw) {
            ...
            materialized_tw = smallest_known_treewidth;
        }
    };
    ...
    while (!open_cells.empty()) {
        ...
        // top() is const&, so cast away const to move (not copy) the cell out
        // before pop discards it.
        auto current_cell = std::move(const_cast<Cell&>(open_cells.top()));
        open_cells.pop();
        ...
        if (current_cell.bag_size() > max_closed_bag_size) {
            ...
            for (int x = 0; x < interior_node_count; ++x) {
                if (!nodes_of_representative[x].empty()) {
                    ...
                    for (auto& y : new_cell_interior_node_list)
                        y = sub_node_to_node(y);
...
IFlowCutter::IFlowCutter(int n, int m, IFlowCutterConfig cfg) :
        nodes(n), best_bag_size(numeric_limits<int>::max()), head(2*m, n), tail(2*m, n),
        cfg(cfg) {}

void IFlowCutter::importGraph(const PrimalGraph& g)
{
    int next_arc = 0;
...
void IFlowCutter::test_new_order(const ArrayIDIDFunc& order, TreeDecomposition& td) {
    ...
    if (x < best_bag_size) {
        best_bag_size = x;
        td = output_tree_decomposition_of_order(tail, head, order);
    }
}

TreeDecomposition IFlowCutter::output_tree_decomposition_of_order(ArrayIDIDFunc tail, ArrayIDIDFunc head, const ArrayIDIDFunc& order) {
    ...
    {
        std::vector<int> btail, bhead, weight;
        ...
        int arc_count = btail.size();

        auto out_arc = invert_id_id_func(
            id_id_func(
                arc_count, bag_count,
                [&](unsigned a) { return btail[a]; }
            )
        );
...
TreeDecomposition IFlowCutter::output_tree_decomposition_of_multilevel_partition(const ArrayIDIDFunc& to_input_node_id, const std::vector<Cell>& cell_list) {
    TreeDecomposition better_td;
...
{
    // Tree-decomposition time limits use CPU time (not wall-clock): the compute
    // budget (total_limit) and the improvement grace period stay independent of
    // machine load / concurrent processes, keeping the chosen TD reproducible.
    const double t_begin = cpu_sec();

    auto should_stop = [&]() -> bool {
        return cfg.total_limit > 0 && cpu_sec() - t_begin > cfg.total_limit;
    };
    ...
    try {
        {
            // Upstream computes a DFS preorder, then overwrites it with the
            // identity, so the relabeling is a no-op. Build the identity directly.
        ...
        auto on_new_multilevel_partition = [&](const std::vector<Cell>& multilevel_partition) {
            td = output_tree_decomposition_of_multilevel_partition(preorder, multilevel_partition);
            best_bag_size = get_treewidth_of_multilevel_partition(multilevel_partition);
        };
        ...
        try {
            ...
            if (node_count > 500000) {
                ...
                compute_multilevel_partition(tail, head, flow_cutter::ComputeSeparator(config), best_bag_size, should_stop, on_new_multilevel_partition);
            }
            ...
            if (node_count < 10000
                    && (!cfg.run_min_shortcut || cfg.run_min_shortcut(best_bag_size - 1))) {
...

            {
                ...
                for (int i = 2;; ++i) {
                    if (cfg.iters > 0 && i >= cfg.iters) break;
                    ...
                    // With no time budget and no pass cap, run a single pass so
                    // the library returns promptly by default.
                    if (cfg.total_limit <= 0 && cfg.iters <= 0)
                        break;
                }
            }
        } catch (...) {
